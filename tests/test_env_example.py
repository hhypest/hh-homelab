...
# ---------------------------------------------------------------------------
#  Новая обязательная переменная останавливает обновление
# ---------------------------------------------------------------------------
#  Замечание Codex. Файл .env в git не лежит и обновлением не трогается,
#  а в compose каждая переменная объявлена как ${VAR:?…} — без значения
#  Compose не запускается вовсе. Значит переменная, появившаяся в новом
#  выпуске, останавливает обновление на полпути: образы скачаны, стек
#  не поднимается, и виноватым выглядит обновление, а не отсутствующая
