...
PERSONAL = [
    ...
    # Конфигурация sing-box — файл с учётными данными, а не с настройками:
    # по UUID подключаются к серверу VLESS, по токену — к локальному API
    # роутера. В документацию попадают её фрагменты, и вместе с ними легко
    # уезжает боевое значение. UUID из одних нулей пропускаем — он для того
    # и заведён, как заглушки AA:BB:CC выше.
    (re.compile(r'"uuid"\s*:\s*"(?!0{8}-)[0-9a-f]{8}(-[0-9a-f]{4}){3}-[0-9a-f]{12}"', re.I),
