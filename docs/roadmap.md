...
| 2 | [Датчик протечки](#zigbee-и-настоящий-умный-дом) | Единственный пункт, который может окупиться деньгами | S |
...
## Zigbee и настоящий умный дом

Самое большое неосвоенное поле. Сейчас Home Assistant — панель мониторинга
NAS с пультом от телевизора. Датчики превращают его в то, чем он задуман.

- [ ] **Zigbee-координатор в USB DS725+ и Zigbee2MQTT.**

      Плюс контейнер Mosquitto. Оба лёгкие, но это ещё два контейнера
      и +2 к числу в `docker_state.py` и `docker.yaml` — не забыть.

      Две ловушки, о которых узнают уже после покупки:

      Стик нельзя втыкать вплотную к разъёму USB 3.0 — он гадит помехами
      ровно в диапазоне 2,4 ГГц. Нужен USB-удлинитель хотя бы на метр,
      это не блажь.

      Канал Zigbee выбирается подальше от канала Wi-Fi 2,4 ГГц, на котором
      работает сеть IoT. Иначе датчики начнут «терять» события, и искать
      причину вы будете долго.

      **Повод:** решение поставить хоть один датчик. **Усилия:** M.

- [ ] **Датчик протечки под мойку и за стиральную машину.**

      Из всего списка это единственный пункт, который способен окупиться
      буквально — одним предотвращённым заливом соседей. Стоит как обед,
      живёт на батарейке пару лет, автоматизация в три строки: протечка →
      🔴 в Мониторинг + красный экран на телевизоре, если он включён.

      **Повод:** сразу после координатора. **Усилия:** S.

- [ ] **Температура и влажность по комнатам.**

      Кроме уюта — практическая польза: понять, действительно ли NAS греется
      сам по себе, или это в комнате летом тридцать градусов. Сейчас
      температура диска ни с чем не сопоставима.

      **Повод:** первое лето с мониторингом. **Усилия:** S.

- [ ] **Розетка с измерением потребления на стиральную машину.**

      Классика, которая на удивление хорошо работает: потребление упало ниже
      порога и держится десять минут → «стирка закончилась» в Пачку или
      на телевизор. Тем же способом видно, что забыли утюг.

      **Повод:** появление координатора. **Усилия:** S.
...
- [ ] **Присутствие дома.**
      ...
      **Повод:** когда датчиков станет больше двух. **Усилия:** M.

- [ ] **ИБП с USB.**

      Synology увидит его сам, Home Assistant прочитает статус через
...
- [ ] **Sonarr для сериалов.**
      ...
      `docker_state.py`, в HTTP-проверки и на дашборд. Тест
      `test_список_по_умолчанию_совпадает_с_compose` не даст забыть половину.
...
flowchart LR
    ...
    PINS --> ZIGBEE[Zigbee-координатор]
    ZIGBEE --> LEAK[Датчик протечки]
    ZIGBEE --> CLIMATE[Климат по комнатам]
    LEAK --> UPS[ИБП]
    PINS --> SONARR[Sonarr]
