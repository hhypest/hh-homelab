...
- [ ] **Sonarr для сериалов.**
      ...
      `test_watched_matches_compose_files`: он сверяет надзор
...
Подробный разбор с ценами, арифметикой разрешения и двумя условиями,
при которых ответ изменится, — в [обзоре проекта](overview.html), раздел 4.
Дешёвый первый шаг, если детализации не хватает: `scan_interval: 300`
в `packages/docker.yaml` вместо нынешнего часа.
