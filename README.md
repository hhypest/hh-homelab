...
| **[Дорожная карта](https://hhypest.github.io/hh-homelab/roadmap.html)** | Куда это может расти дальше, в каком порядке и чего решено не делать | 20 пунктов с отметками ([исходник](docs/roadmap.html)) |
...
Абсолютным остался только `/volume1/data`: это не конфигурация, а библиотека,
и монтируется она одним томом целиком — иначе Radarr не сможет импортировать
хардлинком. Подробнее, с обоснованием, —
