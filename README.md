...
| **[Настройка медиа-стека](https://hhypest.github.io/hh-homelab/media-stack.html)** | Как связать шесть приложений в конвейер «запрос → фильм в библиотеке» | 37 шагов с отметками ([исходник](docs/media-stack.html)) |
...
только этот README. У чек-листов и дорожной карты отметки сохраняются
в браузере, прогресс виден в боковой панели. Ссылки заработают после
включения GitHub Pages:
*Settings → Pages → Source: Deploy from a branch → main → /docs*. До этого
файлы открываются прямо из репозитория.
...
| Симптом | Раздел |
|---|---|
| Фильмы занимают вдвое больше места, чем должны | [медиа-стек, разделы 1 и 8.2](docs/media-stack.html) |
| Radarr ничего не находит | [медиа-стек, раздел 10](docs/media-stack.html) |
...
| Интернет «залипает» во время загрузок | [медиа-стек, шаг 2.5](docs/media-stack.html) |
...
Подробная инструкция с проверками на каждом шаге и разбором типичных
ошибок — в [обзоре проекта](https://hhypest.github.io/hh-homelab/overview.html),
раздел 5. Здесь — короткая выжимка.
...
# 2. Каталоги состояния — их нет в git, но они должны существовать
#    ДО первого запуска: иначе Docker создаст их от root, и контейнеры
#    с PUID=1026 не смогут в них писать
sudo mkdir -p media/config/{qbittorrent,prowlarr,radarr,jellyfin,seerr} \
              media/cache/jellyfin media/watch
...
# 4. Поднять
cd media && sudo docker compose up -d && cd ..
cd homeassistant && sudo docker compose up -d && cd ..
```

**Клон — это и есть рабочая конфигурация, а не её копия.** Оба стека
монтируют своё состояние относительными путями: `./config` у Home Assistant,
`./config/<сервис>` у медиа-стека. Compose разрешает такие пути от каталога
самого `compose.yaml`, поэтому весь домашний сервер переносится вместе
с репозиторием: склонировали в другое место — и он поднимается там же,
без единой правки путей. Заодно всё, что Home Assistant пишет
в конфигурацию, сразу попадает в `git status`, и вопрос «какая версия
свежее» не возникает.

Абсолютным остался только `/volume1/data` — библиотека на терабайты.
Это не конфигурация, она живёт своей жизнью, и монтируется одним томом
целиком в qBittorrent и Radarr: только так Radarr импортирует хардлинком,
а не копией. По той же логике в `homeassistant/compose.yaml` относителен
лишь `./config`, а `/etc/localtime` и docker-сокет записаны абсолютно.

> **Переезжаете с прежней схемы?** Раньше медиа-стек монтировал
> `/volume1/docker/<сервис>/config`. Готовые базы и настройки надо
> перенести — порядок действий с проверками в
> [обзоре проекта](https://hhypest.github.io/hh-homelab/overview.html),
> раздел 5.3. Коротко: остановить стек, `mv` каталоги на новое место,
> вернуть владельца, поднять.
...
дашбордов. Дорога здесь не память — после планки на 8 ГБ её хватит с избытком,
...
память до 12 ГБ, том на NVMe, ИБП по USB.
