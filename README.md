...
Обновления вместо этого приходят пул-реквестом от Dependabot, раз в неделю,
одной группой на оба стека. Порядок поэтому обратный привычному: сначала
прочитать changelog и смержить PR, потом `git pull` на NAS, и только потом
...
проверки и на плавающем теге, и на образе вовсе без тега, и на каталоге,
выпавшем из `dependabot.yml`, — закрепление без того, кто приносит
обновления, оставило бы вас на версиях того дня, когда его сделали.
Проверяются и образы модов из `DOCKER_MODS`, а не только ключи `image`.

> **VueTorrent придётся обновлять руками.** Dependabot разбирает в compose-файле
> только `image`, а `DOCKER_MODS` для него обычная переменная окружения. Мод
> при этом скачивается при **каждом старте контейнера**, а не при
> `compose pull`, — так что с плавающим тегом версия менялась бы даже
> от простого перезапуска, причём `pull` этого не показал бы вовсе. Теперь
> она зафиксирована, но новые релизы придётся отслеживать самому:
...
| **Python · ruff и тесты** | линтер и 141 тест на Python 3.11, 3.12 и 3.13; среди них — проверка самого workflow: кэш pip знает, где лежат зависимости, и версии действий не разъехались | `ruff`, `pytest` |
...
| **Документация** | целостность семи HTML-страниц, живые ссылки, отсутствие личных данных, сверка обещанных в этой таблице счётчиков шагов с самими страницами | [`validate_docs.py`](scripts/validate_docs.py) |
