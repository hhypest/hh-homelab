...
Обновления вместо этого приходят пул-реквестами от Dependabot, раз в неделю —
по одному на стек. Порядок поэтому обратный привычному: сначала
...
Но и на Dependabot полагаться целиком не стоит — он уже молчал две недели
подряд, пока пять образов уходили вперёд. Поэтому есть независимая сверка,
которая ни от кого не зависит:
...
В CI её нет намеренно: это поход в сеть наружу, ему не место в проверке
на каждый push.
...
| **Python · ruff и тесты** | линтер и 280 тестов на Python 3.11, 3.12 и 3.13; среди них — проверка самого workflow: кэш pip знает, где лежат зависимости, и версии действий не разъехались | `ruff`, `pytest` |
...
Home Assistant — ни одного пользователя. Всё это закрывается бэкапами,
